...
map1 <- ggplot() +
  ...
  scale_fill_viridis_c(direction = -1, name = "Wastewater \nDischarge \nIndicator")+
  annotation_scale(plot_unit = "km", location = "br", style = "ticks")+
  annotation_north_arrow(location = "br",
                         pad_x = unit(0.2, "in"),
                         pad_y = unit(0.2, "in"),
                         style = ggspatial::north_arrow_nautical(text_size = 20))+
  theme_minimal()+
...
bg_holc_summary <- bg_holc %>% 
  group_by(holc_grade) %>% 
  summarize("mean_lowincpct" = round(mean(LOWINCPCT, na.rm = TRUE), digits = 2),
...
table1 <- bg_holc_summary %>% 
  kable(col.names = c("HOLC Grade", "% Low Income", "PM 2.5 Percentile", "Low Life Expectancy Percentile", "Air Toxics Cancer Risk Percentile"), caption = "Mean values of four EJScreen Indicators within each HOLC grade.", escape = FALSE) %>% 
  kable_minimal()

table1
# kable(x = bg_holc_summary , col.names = c("HOLC Grade", "% Low Income", "PM 2.5 Percentile", "Low Life Expectancy Percentile", "Air Toxics Cancer Risk Percentile"), caption = "Mean values of 4 EJScreen Indicators within each HOLC grade.")
```
...
#read in birds data
birds <- st_read("./data/gbif-birds-LA/gbif-birds-LA.shp")
...
plot1 <- ggplot(data = birds_redlining_summary) +
  geom_col(aes(x = holc_grade, y = perc, fill = holc_grade)) +
  geom_text(aes(x = holc_grade, y = perc, label = round(as.numeric(perc))),vjust = -0.5)+
  scale_fill_manual(values = c("#9C964A", "#F3DF6C", "#DC863B", "#C93312"))+
  labs(y = "Percent of Total Observations (%)", x = "HOLC \nGrade", title = "Percent of Bird Observations in 2022 by HOLC Grade Category", fill = "HOLC Grade")+
  theme_minimal()
